...
public:
	...
	// DELETE
	std::unordered_map<std::string, ValueType> print()
	{
	...
	void printPath(std::string key)
	{
		...
		while (index < key.size())
		{
			...
			std::cout << childNode->m_string << std::endl;
...
private:
	...
	// DELETE
	void nPrint(std::unordered_map<std::string, ValueType>& map, Node* node, std::string s)
	{
		if (node == nullptr) return;
		s += node->m_string;
		if (node->m_isWord == true) map[s] = node->m_value;
		if (node->m_children.empty()) return;
		for (Node* children : node->m_children)
		{
			nPrint(map, children, s);
		}
	}
...
void RadixTree<ValueType>::insert(std::string key, const ValueType& value)
{
	...
	while (index < key.size())
	{
		...
		if (childNode == nullptr)
		{
			curr->m_children.push_back(new Node(currStr, copy, true));
		...
		if (indexToSplit == -1)
		{
			...
			if (currStr.size() < childNode->m_string.size())
			{
				...
				Node* addedNode = new Node(currStr, copy, true);
		...
		else
		{
			...
			if (childNode->m_children.size() == 0)
			{
				childNode->m_children.push_back(new Node(suffixStr, childNode->m_value, true));
				childNode->m_children[0]->m_children.push_back(nullptr);
			}
			else
			{
				Node* toInsert = new Node(suffixStr, childNode->m_value, false);
				typename std::vector<Node*>::iterator it = childNode->m_children.begin();
				while(it != childNode->m_children.end())
				{
					toInsert->m_children.push_back(*it);
					it = childNode->m_children.erase(it);
				}
				childNode->m_children.push_back(toInsert);
			}
		}
		curr = childNode;
		index += indexToSplit;
